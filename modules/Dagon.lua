...
Dagon.MenuPath = {"Utility", "Items Helper", "Dagon"}

Dagon.optionEnable = Menu.AddOptionBool(Dagon.MenuPath, "AutoDagon", false)

function Dagon.OnUpdate()
	if not Menu.IsEnabled(Dagon.optionEnable) then return end
	...
	if dagon ~= nil and Ability.IsReady(dagon) and Ability.GetManaCost(dagon) <= NPC.GetMana(myHero) then
		local damageDagon = Ability.GetLevelSpecialValueForFloat(dagon, "damage")
		for _,hero in pairs(Heroes.GetAll()) do
			if hero ~= nil and NPC.IsEntityInRange(myHero,hero,Ability.GetCastRange(dagon)) and not Entity.IsSameTeam(hero,myHero) then
				if Entity.IsAlive(hero) and not NPC.IsIllusion(hero) and Dagon.IsHasGuard(hero) == "nil" then
					local totaldomage = Dagon.GetDamageDagon(myHero,hero,damageDagon)
...
function Dagon.GetDamageDagon(mynpc,target,dmg)
	...
	if bristleback and Ability.GetLevel(bristleback) ~= 0 then
		...
		local orig = -1 * (Dagon.Atan2(vectortarget:GetY()-vectormy:GetY(), vectortarget:GetX()-vectormy:GetX())/math.pi*180) + taorig
